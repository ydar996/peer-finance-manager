...
const workbookPath =
  process.argv[2] ||
  path.join(__dirname, "..", "Assurance Status 2 2026.xlsx");
const outputDir = path.join(__dirname, "..", "statements", "2026-02");
const sheetName = "February 2026";
const statementMonthLabel = "February 2026";
...
function parseWorkbook() {
  ...
  return memberRows.map((row) => {
    ...
    const february2026Deposit = getMonthValue(row, 2026, "February");
    const totalDeposits =
...
function buildHtml(member, preparedOn) {
  ...
  const monthRows = [
    ...
    ["February 2026 Deposit", formatMoney(member.february2026Deposit)],
  ]
    .map(
      ([label, value]) => `
        <tr>
