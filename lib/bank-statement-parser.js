...
function parseBankStatementCsv(filePath, memberNames = []) {
  ...
  const memberCol = headerCols.findIndex((c) => /^Member$/i.test(String(c || "").trim()));
  const narrativeCol = headerCols.findIndex((c) => /^Narrative$/i.test(String(c || "").trim()));
  ...
  for (let i = headerIndex + 1; i < lines.length; i++) {
    ...
    const narrative =
      (narrativeCol >= 0 ? cols[narrativeCol] : cols[4] || "").trim();
    const date = parseDate(dateStr);
    const amount = parseAmount(amountStr);
    if (!date || amount == null) continue;

    const memberFromCol =
      memberCol >= 0 ? matchMemberName(cols[memberCol], memberNames) : null;
    ...
    const entry = {
      ...
      amount,
      narrative,
      member: memberFromCol || resolveMember(description, memberNames),
    };
