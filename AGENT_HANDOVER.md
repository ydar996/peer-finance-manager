...
- **2026-07-11** — **Books refresh + reclassify hotfix:** Cooperative Books reloads when switching from Import (no hard refresh needed). Bank import/append and reclassify/split also prefetch books data. Fixed `clearPortfolioInterestShareCache is not a function` on reclassify (export from `loan-ledger-service.js`). Files: `app.js`, `loan-ledger-service.js`. **Production:** `git push`.
