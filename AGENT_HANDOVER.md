...
- **2026-07-06** — **Admin delete membership applications:** Forms & Documents → Membership Applications **Delete** removes FlexxForms submission row; also deletes linked **pending approval** prospective member (profile + portal login) when no ledger transactions or loans. Blocks approved apps and members with activity. **Production:** `git push`.
...
| 7 | **FlexxForms: verify submit + field import after parser fix** | Submit works (first successful submission July 6). Parser fix deployed for wrong name/missing email — user should **Reprocess Data** on existing application after `git push`, then verify new submissions import correctly. |
